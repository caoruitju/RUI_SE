...
class DPCRNPLUS(nn.Module):
    def __init__(self, rnn_hidden=128, win_len=512, hop_len=128, fft_len=512, win_type='hanning',
        ...
        self.kernel_size = ([5, 2], [5, 2], [5, 2], [5, 2], [5, 2], [5, 2])
        ...
        hidden_dim = 4
        self.enhance = dprnn_block.DPRnn(input_ch=kernel_num[-1], F_dim=hidden_dim, hidden_ch=kernel_num[-1])
    ...
    def forward(self, x):
        ...
        imag = est_mags * torch.sin(est_phase)# B,257,T

        '''multiple refinement iterations'''
        refinement_out = torch.stack([real[:,:-1,:], imag[:,:-1,:]], dim= 1)
        residual = torch.stack([real[:,:-1,:], imag[:,:-1,:]], dim= 1) # B,2,256,T
